...
$(document).ready(function () {
  ...
  $('#send_message').click(function (e) {
    ...
    // Supabase 전송
    submitLoanToSupabase();
  });

  $('#name,#phone,.intro_select,.intro_select1,.intro_select2,.intro_select3,.intro_select4,.intro_select6,#location,#message,#agree11').bind('keyup click change', form_c);
});

// ================================================================
//  Supabase 전송
// ================================================================
function submitLoanToSupabase() {
  var payload = {
    job_type:      getSelectedValue('intro_select1') || '',
    collateral:    getSelectedValue('intro_select2') || '',
    loan_amount:   getSelectedValue('intro_select') || '',
    loan_type:     getSelectedValue('intro_select6') || '',
    no_disqualify: getSelectedValue('intro_select3') || '',
    recent_loan:   getSelectedValue('intro_select4') || '',
    name:          $.trim($('#name').val() || ''),
    phone:         String($('#phone').val() || '').replace(/[^0-9]/g, ''),
    call_time:     $('#location').val() || '',
    message:       $.trim($('#message').val() || '') || null,
    source:        '당근'
  };

  fetch(SUPABASE_URL + '/rest/v1/loan_consultations', {
    method: 'POST',
    headers: {
...
function form_c() {
  ...
  if (!position2) return disableButton('담보 여부 선택을 확인하세요');
  if (!position)  return disableButton('대출금액 선택을 확인하세요');
  if (!position6) return disableButton('대출유형 선택을 확인하세요');
  if (!position3) return disableButton('4번 신청불가 여부를 확인하세요');
  if (!position4) return disableButton('대출유무 선택을 확인하세요');

  if (!regex1.test(name) || name.length <= 1) return disableButton('성함 입력을 확인하세요');
  if (!(phone.substr(0, 3) == '010' && phone.length == 11 && regex.test(phone))) return disableButton('휴대전화번호 입력을 확인하세요');
  if (!location) return disableButton('통화가능시간 선택을 확인하세요');
